...
public class CassandraBasicIT {
    ...
    public static void staticSetup() {
        ...
        session.execute("CREATE TABLE IF NOT EXISTS test_counter_table (k bigint, name text, v counter, PRIMARY KEY (k, name) )");
    }
    ...
    public void insertAndReadBackAsync() {
        ...
        final long now = 100L;//System.currentTimeMillis();
        String text = "saleh";
        final long count = 1L;
        final PreparedStatement insertPS = session.prepare("UPDATE test_counter_table SET v=v+? WHERE k=? AND name=?");
        final ResultSetFuture mutationFuture = session.executeAsync( new BoundStatement(insertPS).bind(count, now, text) );
